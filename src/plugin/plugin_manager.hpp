...
public:
    ...
    int do_request_plugins(int plugin_type, int call_type, int area, const data_entry *key, const data_entry *value,
                           plugins_root *&root);

    // the last parameter root is the value got when the doRequestPlugIns be called.
    void do_response_plugins(int rev, int plugin_type, int call_type, int area, const data_entry *key,
                             const data_entry *value, plugins_root *&root);
