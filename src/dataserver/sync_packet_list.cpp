...
bool sync_packet_list::enable_disk_save(const std::string& disk_save_path,bool replay)
{
  log_info("enable disk save to %s",disk_save_path.c_str());
  m_use_disk=true;
  //open file with the only file.
  m_disk_path=disk_save_path+LOG_FILE_NAME;
  if(!m_disklist_file.start(m_disk_path,replay))
  {
    log_error("open disk file failed with %s",disk_save_path.c_str());
    return false;
  }
  ...
  if(_size>0)
  {
    log_info("load %d record from file %s",_size,disk_save_path.c_str());
...
int sync_packet_list::do_batch_load(unsigned int _want_num)
{
  ...
  std::vector<request_sync *> _vecotr_node;
  bool bneed_switch=!m_disklist_file.batch_get(_want_num,_vecotr_node);

  if(bneed_switch||_vecotr_node.size()<_want_num)
  {
    atomic_set(&m_disk_stored, STATE_MEM_STORE);
    //it's lock free,so we should reget it to fetch the new one.
    bneed_switch=m_disklist_file.batch_get(_want_num-_vecotr_node.size(),_vecotr_node);
    int _store_stated= atomic_read(&m_disk_stored);
    if(STATE_MEM_STORE==_store_stated && bneed_switch)
    {
      log_info("load %d from file finished,switch to memory",_vecotr_node.size());
      m_disklist_file.clear();
    }
  }

  log_info("load %d from file ,want %d",_vecotr_node.size(),_want_num);
  for(unsigned int i=0;i<_vecotr_node.size();i++)
  {
    //m_pout_queue dones't care sequece.
    m_pout_queue->put(_vecotr_node[i]);
  }
  return _vecotr_node.size();
}


int sync_packet_list::put(request_sync *& value)
{
...
int sync_packet_list::put_head(request_sync *& value)
{
  //i will put to head if necessary.
  return this->put(value);
}

request_sync * sync_packet_list::get(int tmms)
{
  request_sync  *pnode=m_pout_queue->get(tmms);
