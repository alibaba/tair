...
std::string DecodeLdbString(const Slice& value) {
  std::string str;
  int32_t bucket = LdbKey::get_bucket_number(value.data());
  int area = LdbKey::decode_area(value.data() + LDB_KEY_AREA_OFFSET);
  char buf[64];
  memset( buf , 0 , sizeof(buf));
  sprintf(buf, "bucket:%d, area:%d, client_user_key:" , bucket, area);
  str.append(buf);

  for (size_t i = LDB_KEY_CLIENT_USER_KEY_OFFSET; i < value.size(); i++) {
    char c = value[i];
    if (c >= ' ' && c <= '~') {
      str.push_back(c);
    } else {
...
bool DumpTable(Env* env, const std::string& fname) {
  ...
  for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
    ...
    if (!ParseInternalKey(iter->key(), &key)) {
      ...
      printf("'%s' @ seq: %8llu : %s => '%s'\n",
             DecodeLdbString(key.user_key).c_str(),
