...
void table_builder1::caculate_capable() {
    ...
    for (server_list_type::const_iterator it = available_server.begin();
        ...
        if (last_node_count <=
            pre_node_count_min) {        //try my best to make every data sever handle tokenPerNode_min buckets.
        ...
        if (last_mnode_count <=
            pre_mnode_count_min) {        //try my best to make every data sever handle tokenPerNode_min buckets.
            int mmin_s = i - mmax_s;
