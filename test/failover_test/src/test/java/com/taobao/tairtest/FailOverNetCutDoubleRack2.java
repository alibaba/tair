...
public class FailOverNetCutDoubleRack2 extends FailOverBaseCase{
	...
	public void testFailover_01_cutoff_a1b1_a1b2_b1a2()
	{
		...
		if(!execute_tool_on_mac(local, put))fail("put fail on client!");
		...
		int migrateCount1 = check_keyword(csList.get(0), start_migrate, tair_bin+"logs/config.log");
		int migrateCount2 = check_keyword(csList.get(1), start_migrate, tair_bin+"logs/config.log");
		...
		migrateCount1=check_keyword(csList.get(0), start_migrate, tair_bin+"logs/config.log");
		migrateCount2=check_keyword(csList.get(1), start_migrate, tair_bin+"logs/config.log");

		//1st write data while shut off net
		if(!execute_tool_on_mac(csList.get(0), put))fail("put fail on master cs!");
		if(!execute_tool_on_mac(csList.get(1), put))fail("put fail on slave cs!");
		if(!execute_tool_on_mac(local, put))fail("put fail on client!");
		...
		if(!execute_tool_on_mac(csList.get(0), get))fail("get fail on master cs!");
		if(!execute_tool_on_mac(csList.get(1), get))fail("get fail on slave cs!");
		if(!execute_tool_on_mac(local, get))fail("get fail on client!");
		wait_tool_on_mac(csList.get(0));
		wait_tool_on_mac(csList.get(1));
		wait_tool_on_mac(local);
	...
	public void testFailover_04_cutoff_a1b2_b1a2_a1b1()
	{
		...
		init_migrateCount1 = check_keyword(csList.get(0), start_migrate, tair_bin+"logs/config.log");
		init_migrateCount2 = check_keyword(csList.get(1), start_migrate, tair_bin+"logs/config.log");
	...
	public void testFailover_06_cutoff_b1a2_a1b2_a1b1()
	{
		...
		if(!execute_tool_on_mac(local, put))fail("put fail on client!");
		if(!execute_tool_on_mac(csList.get(0), put))fail("put fail on master CS!");
		if(!execute_tool_on_mac(csList.get(1), put))fail("put fail on Backup CS!");
		wait_tool_on_mac(local);
		wait_tool_on_mac(csList.get(0));
		wait_tool_on_mac(csList.get(1));
...
//		waitto(ds_down_time/4);
	...
	public void setUp()
	{
		...
		if(!batch_modify(dsList2, tair_bin+groupconf, copycount, "2"))
			fail("modify configure file failure");
	}
