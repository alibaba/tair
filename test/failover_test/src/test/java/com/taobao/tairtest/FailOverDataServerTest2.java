...
/*	@Test
	...
	@Test
	public void testFailover_28_kill_more_ds_util_one_left_then_restart()
	{
		log.error("start DataServer test Failover case 28");
		int waitcnt=0;
		...
		//close some ds
		if(!batch_control_ds(dsList.subList(0, dsList.size()-1), FailOverBaseCase.stop, 0))
		...
		assertTrue("put successful rate small than 90%!",datacnt1/FailOverBaseCase.put_count_float>0.9);
		log.error("Write data over!");
		...
		//verify get result
		assertEquals("get data verify failure!", datacnt+datacnt1, getVerifySuccessful());
		log.error("Successfully verified data!");
		
		//end test
		log.error("end DataServer test Failover case 28");
	}
